    ...
    function __construct($options = array()) {
        ...
        if (empty($options)) {
            $options = array(
                ...
                'user' => C('MEMCACHE_USER'),
                'password' => C('MEMCACHE_PASS'),
                'weight' => 0,
        ...
        $key = md5($this->options['host'] . $this->options['port']);
        ...
        if (!count($this->handler->getServerList())) {
            ...
            $this->handler->setSaslAuthData($this->options['user'],$this->options['password']);
        }
